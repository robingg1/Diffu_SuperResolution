...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if use_spatial_transformer:
            assert context_dim is not None, 'Fool!! You forgot to include the dimension of yourcd c cross-attention conditioning...'
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        hs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=True)
        ...
        for module in self.output_blocks:
            hs_pop = hs.pop()
            if h.shape[2] != hs_pop.shape[2] or h.shape[3] != hs_pop.shape[3]:
                h = F.interpolate(h, (hs_pop.shape[2],hs_pop.shape[3]), mode = 'bilinear')
...
class PointEncoder(nn.Module):
    def __init__(
        ...
        self.output = nn.Sequential(
            nn.Conv1d(self.hidden_channels,1,3,1,1),
            nn.SiLU(),
        )
...
class OneDUnet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.pointencoder = PointEncoder(3,1)
        self.xt_dim = xt_dim
        ...
        self.time_embed = nn.Sequential(
            nn.Linear(self.latent_dim, self.time_dim),
            nn.SiLU(),
            nn.Linear(self.time_dim, self.time_dim),
        )
        ...
        self.concat_decoder = nn.Sequential(
            nn.Linear(self.xt_dim+self.time_dim,self.cond_dim,(self.xt_dim+self.time_dim,self.cond_dim)/2),
            nn.SiLU(),
            nn.LayerNorm((self.xt_dim+self.time_dim,self.cond_dim)/2),
            nn.Linear((self.xt_dim+self.time_dim,self.cond_dim)/2,self.xt_dim),
            nn.SiLU(),
        ...
        self.out = zero_module(nn.Linear(self.xt_dim, self.xt_dim))

    def forward(self, x, time_steps, cond, use_attention=True):
        time_embed = self.time_embed(timestep_embedding(time_steps,self.xt_dim))
        x_embed = self.joint_embed(x)
        cond_embed = self.pointencoder(cond)
        if use_attention:
          for module in self.temporal_decoder_blocks:
             x_embed = module(x_embed,time_embed,cond_embed)
        else:
            concat_x = torch.concat((x_embed,cond_embed,time_embed),0)
